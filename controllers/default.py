...
def get_knv_metadata():
    if not db(
            db.t_knv_metadata.submission_id == request.args(0)).select(
            db.t_knv_metadata.submission_id):
        redirect(URL('error'))
    return dict(
        data=db(
            db.t_knv_metadata.submission_id == request.args(0)).select())
...
def create_knv_metadata():
    if len(db(db.t_knv_metadata.submission_id == request.args(
            0)).select(db.t_knv_metadata.submission_id)) > 0:
...
def get_onix_metadata():
    ...
    authors_pre = db(
        db.authors.submission_id == submission_id).select(
        db.authors.last_name,
        db.authors.first_name,
        db.authors.user_group_id,
        db.authors.country,
    ...
    title_text = db(
        (db.submissions.submission_id == submission_id) & (
            ...
            db.series_settings.locale == locale) & (
                db.series_settings.setting_name == 'title')).select(
                    db.series_settings.setting_value).first()['setting_value']
...
def create_onix_metadata():
    if (request.args(0) is None) | (len(db(db.t_onix_additionals.submission_id ==
                                           request.args(0)).select(db.t_onix_additionals.submission_id)) > 0):
    ...
    published_submissions = crud.select(
        db.published_submissions,
        db.published_submissions.submission_id == book_id)
    publication_formats = crud.select(
        db.publication_formats,
        db.publication_formats.submission_id == book_id)
    submissions = crud.select(db.submissions,
                              db.submissions.submission_id == book_id)
    submissions = crud.select(db.submissions,
                              db.submissions.submission_id == book_id)
    submission_settings = crud.select(
        db.submission_settings,
        db.submission_settings.submission_id == book_id)
    return dict(form=form,
...
def return_published_books():
    query = (
        ...
        db.submission_settings.setting_name == 'title') & (
            db.t_onix_additionals.submission_id == db.submission_settings.submission_id)
    entries = SQLFORM.grid(
        query,
        fields=[
            db.submission_settings.submission_id,
            db.submission_settings.setting_value,
            db.submissions.date_submitted],
        field_id=None,
        left=None,
        headers={},
        ...
        links=[
            dict(
                header=T('ONIX 2.1 :  Softcover'),
                body=(
                    lambda row: A(
                        'ONIX',
                        _href=T(
                            '../onix/get_version.xml/' + str(
                                row.submission_settings.submission_id) + '/Softcover')))),
            dict(
                header=T('KNV'),
                body=(
                    lambda row: A(
                        'XML',
                        _href=T(
                            'get_knv_metadata.xml/' + str(
                                row.submission_settings.submission_id))))),
        ],
        links_in_grid=True,
        upload='<default>',
        args=[],
...
def return_unpublished_books():
    ...
    unpublished_books = []
    all_submissions = db(
        (db.publication_formats.submission_id == db.submissions.submission_id) & (
        ...
        groupby=db.submission_settings.submission_id,
        orderby=db.submission_settings.setting_value)
    for row in all_submissions:
        if len(db(db.t_onix_additionals.submission_id ==
                  row.submission_settings.submission_id).select()) <= 0:
            unpublished_books.append(row)

    unpublised_query = (
        db.submission_settings.submission_id == (
            db.t_onix_additionals.submission_id)) | (
        db.submission_settings.submission_id != (
            db.t_onix_additionals.submission_id))

    onix_view = dict(header=T('ONIX'), body=(lambda row: A('ONIX', _href=T(
        'get_onix_metadata/' + str(row.submission_settings.submission_id))))),
        ...
        links=[
            dict(
                ...
                body=(
                    lambda row: '' if len(
                        db(
                            db.t_onix_additionals.submission_id == row.submission_settings.submission_id).select()) > 0 else A(
                        'KNV-XML+ONIX Generieren',
                        _href=T(
                            'create_knv_metadata/' +
                            str(
                                row.submission_settings.submission_id))))),
