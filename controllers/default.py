...
def get_onix_metadata():
     ...
     publication_formats = db(db.publication_formats.submission_id==submission_id).select()
     title_text = db((db.submissions.submission_id==submission_id) & (db.submission_settings.submission_id ==  db.submissions.submission_id) & (db.series_settings.locale==locale) & (db.series_settings.setting_name=='title')).select(db.series_settings.setting_value).first()['setting_value']
...
def create_onix_metadata():
     ...
     '''
     published_submissions = crud.select(db.published_submissions, db.published_submissions.submission_id==book_id)
     publication_formats = crud.select(db.publication_formats, db.publication_formats.submission_id==book_id)
     submissions = crud.select(db.submissions, db.submissions.submission_id==book_id)
     submissions = crud.select(db.submissions, db.submissions.submission_id==book_id)
     submission_settings = crud.select(db.submission_settings, db.submission_settings.submission_id==book_id)
     return dict(form = form,
